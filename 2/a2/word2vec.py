...
def naiveSoftmaxLossAndGradient(
    ...
    centerWordVector = np.transpose(centerWordVec)
    multied_matrix = np.matmul(outsideVectors, centerWordVector)
    y_bar  = softmax(multied_matrix)
    yo_bar = y_bar[outsideWordIdx]
    loss = -np.log(yo_bar)
    V,N = outsideVectors.shape
    y = np.zeros(V)
    y[outsideWordIdx] = 1
    gradCenterVec = np.matmul(y_bar - y, outsideVectors)
    gradOutsideVecs = np.outer(y_bar-y, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    loss = 0.0
    word = outsideVectors[outsideWordIdx]
    loss -= np.log(sigmoid(np.dot(word, centerWordVec)))
    gradOutsideVecs[outsideWordIdx] += centerWordVec * (sigmoid(np.dot(word, centerWordVec)) - 1.0)
    gradCenterVec += word * (sigmoid(np.dot(word, centerWordVec)) - 1.0)
    counter = K
    for counter in range(counter):
        negative_sample = indices[counter+1]
        sample = outsideVectors[negative_sample]
        loss -= np.log(sigmoid(np.dot(-sample, centerWordVec)))
        gradOutsideVecs[negative_sample] -=  centerWordVec * (sigmoid(np.dot(-sample, centerWordVec)) - 1.0)
        gradCenterVec -= sample * (sigmoid(np.dot(-sample, centerWordVec)) - 1.0)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordVec = centerWordVectors[Center_Word_Index]

    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]
        (loss_temp, gradientCenter, gradientOutside) = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += loss_temp
        gradCenterVecs[Center_Word_Index] += gradientCenter
        gradOutsideVectors += gradientOutside
    ### END YOUR CODE
